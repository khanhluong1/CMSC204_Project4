...
public class ConcordanceDataManagerTest {
	...
	public void testCreateConcordanceArraySTUDENT() {
		String myText = "This is a unit test case of testing create concordance array by student.";
		ArrayList<String> words = concordanceManager.createConcordanceArray(myText);
		
		assertEquals(words.get(0),"array: 1");
		assertEquals(words.get(1), "case: 1");
		assertEquals(words.get(2),"concordance: 1");
		assertEquals(words.get(3),"create: 1");
		assertEquals(words.get(4),"student: 1");
		assertEquals(words.get(5),"test: 1");
		assertEquals(words.get(6),"testing: 1");
		assertEquals(words.get(7),"this: 1");
		assertEquals(words.get(8),"unit: 1");
	}
	...
	public void testCreateConcordanceFileSTUDENT() {
		...
		try {
			inputFile = new File("Tes1t.txt");
			PrintWriter inFile = new PrintWriter(inputFile);
			inFile.print("This is the Concordance Data Structure Class.\n" +
					"It is the data structure class that is used with the Concordance Data Manager class.\n" +
					"This is a hash table with buckets. Your hash table with be an\n" +
					"array of linked lists of ConcordanceDataElements. Use the hashcode for an\n" +
			...
			assertEquals("array: 4", words.get(0));
			assertEquals("class: 1,2", words.get(3));
			assertEquals("concordance: 1,2", words.get(4));
			assertEquals("concordancedataelement: 5", words.get(5));
			assertEquals("structure: 1,2", words.get(18));
			assertEquals("use: 4", words.get(22));
			assertEquals("with: 2,3", words.get(24));
			assertEquals("your: 3", words.get(25));
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (Exception e) {
